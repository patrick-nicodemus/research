...
We follow conventions of \cite{CTGDC_1999__40_3_162_0} and
...
and 2-cells} under horizontal composition.
...
$1_x$ and $1_y$. The vertical composition of these 2-cells can then be
strictified by making use of the special isocells $1_x\cong 1_x\cdot
1_x$ and $1_y\cong 1_y\cdot 1_y$; that this composition is strictly
...
\begin{tikzcd}
	...
	\arrow["{1_{Ff}}"{description}, draw=none, from=2-1, to=3-2]
...
\begin{tikzcd}
	...
	\arrow["{1_{Gf}}"{description}, draw=none, from=1-2, to=2-3]
	\arrow["e"{inner sep=.8ex}, "\bullet"{marking}, from=1-3, to=2-3]
...
\end{tikzcd}
  ...
  \end{equation}
  where $\lambda_s : 1\cdot s\cong s$ and $\rho_s: s\cdot 1\cong s$
...
We would like to prove that the induced functor
...
\begin{definition}[Locally split 2-fibration, horizontally split 2-fibration]
  ...
  \mathbf{E}(x,y) -> \mathbf{B}(px,py)$ is split, and
...
\begin{proof}
  ...
  Let us regard $\tau_{\overline f}$ as a 2-cell in
  ...
  horizontally split, then let $\overline{tau}_f$ be the 2-cell
  over $\tau_{\overline(f)}$ with codomain
  $G(f)\circ\overline{\tau}_a$
  given by the splitting. Otherwise, let $\overline{tau}_f$
  ...
  on horizontal arrows in $\mathbb{A}$.

  If $s : a\todot a'$ is a vertical arrow in $\mathbb{A}$,
  then the definition of $\overline{F}s$ and
  $\overline{tau}s$ are analogous to that of $\overline{F}a$
  and $\overline{\tau}a$, we choose $\overline{\tau}_{s}$
  ...
  Similarly, $\overline{F}(\alpha)$ is defined in the same
  way as $\overline{F}(f)$ for $f$ a horizontal morphism.
  Implicitly $\tau$ defines a 2-cell $\overline{\tau}_{\alpha}$
  in $\mathbf{V}_1(p)$ from $\tau_t\circ F(\alpha)\Rightarrow
  pG(\alpha)\circ \tau_s$, and the splitting of
  ...
  $\overline{F}$ defines a true functor on the category of vertical
